...
import { AnimalProfileTabs, type TabConfig } from "@/components/animals/AnimalProfileTabs"
import { validNextStatuses } from "@/lib/statusMachine"
import {
  PawPrint, ArrowLeft, ArrowRight, Clock,
  Heart, FileText, Stethoscope, ExternalLink,
} from "lucide-react"
import type { AnimalStatus, PersonalityProfile, Role } from "@prisma/client"

interface Props {
  params: Promise<{ id: string }>
}

const UPLOAD_ROLES: Role[]  = ["INTAKE_SPECIALIST", "FOSTER_PARENT", "RESCUE_LEAD"]
const HISTORY_ROLES: Role[] = ["RESCUE_LEAD", "INTAKE_SPECIALIST"]
const STAFF_ROLES: Role[]   = ["RESCUE_LEAD", "INTAKE_SPECIALIST", "MEDICAL_OFFICER", "ADOPTION_COUNSELOR"]
...
export default async function AnimalProfilePage({ params }: Props) {
  ...
  const isOwnerFoster    = role === "FOSTER_PARENT" && animal.fosterParentId === session.user.id
  ...
  const showApplicationsLink = role === "ADOPTION_COUNSELOR" || role === "RESCUE_LEAD"

  const canUpload =
    UPLOAD_ROLES.includes(role) &&
    (role !== "FOSTER_PARENT" || isOwnerFoster)

  // Fetch role-gated supplementary data in parallel
  const [fosterParents, historyChangers, activeApplicationsCount] = await Promise.all([
    isRescueLead
    ...
    showHistory && animal.statusHistory.length > 0
      ? prisma.user.findMany({
          where: { id: { in: [...new Set(animal.statusHistory.map((h) => h.changedById))] } },
          select: { id: true, name: true },
        })
      : Promise.resolve([]),
  ...
  const nextStatuses = isRescueLead ? validNextStatuses(animal.status as AnimalStatus) : []
  ...
  if (showProgressNotes) {
    tabs.push({
      ...
      content: <ProgressNotesPlaceholder isOwner={isOwnerFoster} />,
    })
  }
  ...
  if (showMedical) {
    tabs.push({
      ...
      content: <MedicalRecordsPlaceholder />,
    })
  }
  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      <AlertBanner alerts={animal.medicalAlerts} />
        ...
        <>
          ...
          {isRescueLead && (
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-5">
              <h2 className="text-sm font-semibold text-slate-700">Rescue Lead Controls</h2>
                  ...
                  .map((entry, i) => (
                    <li key={i} className="flex items-center justify-between text-sm">
                      <div className="flex items-center gap-2">
                        <span className="text-slate-500 text-xs">{entry.from}</span>
                        <ArrowRight className="w-3 h-3 text-slate-400" />
                        <span className="font-medium text-slate-800 text-xs">{entry.to}</span>
                      </div>
          ...
          {showApplicationsLink && (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-slate-800">Adoption Applications</p>
      ...
      <div className="text-center py-8">
        ...
        {canEdit && (
          <p className="text-xs text-slate-400 mt-1">
            Foster parents can complete the personality profile — coming in Story 10.
          </p>
        )}
      </div>
    )
    ...
    <div className="space-y-4">
      {canEdit && (
        <p className="text-xs text-blue-600 italic">
          Editing personality profiles will be available in Story 10.
        </p>
      )}
      <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
        <div>
          <dt className="text-xs font-medium text-slate-400 uppercase tracking-wide">Energy Level</dt>
          <dd className="text-slate-800 mt-0.5">{profile.energyLevel ?? "—"}</dd>
        </div>
        <div>
          <dt className="text-xs font-medium text-slate-400 uppercase tracking-wide">Good with Kids</dt>
          <dd className="text-slate-800 mt-0.5">
            {profile.goodWithKids === null ? "Unknown" : profile.goodWithKids ? "Yes" : "No"}
          </dd>
        </div>
        <div>
          <dt className="text-xs font-medium text-slate-400 uppercase tracking-wide">Good with Dogs</dt>
          <dd className="text-slate-800 mt-0.5">
            {profile.goodWithDogs === null ? "Unknown" : profile.goodWithDogs ? "Yes" : "No"}
          </dd>
        </div>
        <div>
          <dt className="text-xs font-medium text-slate-400 uppercase tracking-wide">Good with Cats</dt>
          <dd className="text-slate-800 mt-0.5">
            {profile.goodWithCats === null ? "Unknown" : profile.goodWithCats ? "Yes" : "No"}
          </dd>
        </div>
        {profile.traits.length > 0 && (
          <div className="col-span-2">
            <dt className="text-xs font-medium text-slate-400 uppercase tracking-wide mb-1.5">Traits</dt>
        ...
        {profile.idealHome && (
          <div className="col-span-2">
            <dt className="text-xs font-medium text-slate-400 uppercase tracking-wide">Ideal Home</dt>
...
function ProgressNotesPlaceholder({ isOwner }: { isOwner: boolean }) {
  return (
    <div className="text-center py-8">
      <FileText className="w-8 h-8 text-slate-200 mx-auto mb-3" />
      <p className="text-sm text-slate-400">
        {isOwner
          ? "Weekly progress notes will appear here — coming in Story 09."
          : "Progress notes submitted by the foster parent will appear here — coming in Story 09."}
      </p>
    </div>
  )
}

function MedicalRecordsPlaceholder() {
  return (
    <div className="text-center py-8">
      <Stethoscope className="w-8 h-8 text-slate-200 mx-auto mb-3" />
      <p className="text-sm text-slate-400">
        Vaccinations, surgeries, and medications will appear here — coming in Story 11.
      </p>
    </div>
  )
}
